...
do_not_show_plot_just_save = 1
start_node_name = int(config['Start_Node'])  # Initial point, source, the 6 is the initial point in this case, mines 1 to execute the movement for one node to another.
goal_node_name = int(config['End_Node'])  # Final point,sink , the 6 is the initial point in this case, mines 1 to execute the movement for one node to another.

# Pull current time from latest spreadsheet data entry
workbook = xl.load_workbook(config['Current_Spreadsheet'])
worksheet = workbook.active
max_row = worksheet.max_row
time = worksheet.cell(row=max_row, column=1).value
workbook.close()

# Or override it and pick a specific time to display:
#time = 1084  # Set up the time in Seconds

pathData = config['Current_Spreadsheet']
image_path = "Sensor_layout.png"
...
def readPythonSpreadSheet(pathData, parameterIndex=3, time=60):
    ...
    AllParameters = ['Airflow (m3_s)', 'Air Velocity (m_s)', 'Temperature (C)',
                     'Gas (ppm)']  # 'Airflow m³/s','Velocity m/s','Temp Wet Bulb ºC','Visibility m','O2  %' (in this step multiple varibles could be selected, the risks in the emergency)
        ...
        for kn in range(1, len(config['Node_Connections'][k])):
            ...
            for kk in range(len(config['Button_Coordinates'])):
                ...
                if sensor_i == connection_node_i:
                    ...
                    break
            if (sensor_data_connection_index or sensor_data_connection_index == 0) and 'G' in \
                    config['Button_Coordinates'][sensor_connection_i_index][3]:
    ...
    for k in range(len(config['Node_Connections'])):
        ...
        for kn in range(1, len(config['Node_Connections'][k])):
            ...
            if (sensor_data_connection_index or sensor_data_connection_index == 0):
                ...
                sensorSY_in = (-1e4)
                sensorSX_in = (-1e4)
            if (main_sensor_data_i_index or main_sensor_data_i_index == 0) and (
                    sensor_data_connection_index or sensor_data_connection_index == 0) and 'G' in \
                    config['Button_Coordinates'][sensor_connection_i_index][3]:
                grph_i[sensorSX_i, sensorSY_i].update(
                    {(sensorSX_in, sensorSY_in): v_for_algorithm[sensor_data_connection_index]})
        grph.update(grph_i)
    return grph, weights
...
def pathFinderFF(s, t):
    ...
    if rockfalNodes:
        cap[rockfalNodes,
        :] = 0  # For example, in this case the node 22, is blocked for a roof fall, the sensor 22 does not collect and process the data
    ...
    A = [sensorSXY[k, 0] if abs(
        np.arctan((sensorSXY[k, 3] - sensorSXY[k, 1]) / (sensorSXY[k, 2] - sensorSXY[k, 0] + 1e-16))) < 10 else
         sensorSXY[
             k, 2] for k in range(len(sensorSXY))]
    B = [sensorSXY[k, 1] if abs(
        np.arctan((sensorSXY[k, 3] - sensorSXY[k, 1]) / (sensorSXY[k, 2] - sensorSXY[k, 0] + 1e-16))) > 50 else
         sensorSXY[
    ...
    fig = plt.figure();
    plt.imshow(
        img)  # , extent=(-45445.00, -45143.00, -3006062.00, -3005450.00)-45445.00,-45142.00,-3006075.00,-3005450.00
    ...
    colTable = ['green', 'red', 'blue', 'green', 'green', 'green', 'green', 'green', 'green', 'green', 'green',
                'green']  # 'red', 'blue', 'green', 'black', 'yellow', 'cyan', 'brown', 'pink', 'purple', 'none'
    # 14.1.2 Main path
    mainpath = [
        'red']  # 'green', 'green', 'green', 'green', 'red', 'none', 'none', 'green', 'red', 'none', 'none', 'green'
...
def coordinate2nodeSearch(Sx, Sy):
    for k in range(len(config['Button_Coordinates'])):
        if float(config['Button_Coordinates'][k][0]) == float(Sx) and float(
                config['Button_Coordinates'][k][1]) == float(Sy):
...
def plot_path_A_Star(path):
    ...
    # 14.1.1 Alternative path
    colTable = ['green', 'red', 'blue', 'yellow', 'green', 'green', 'green', 'green', 'green', 'green', 'green',
                'green']  # 'red', 'blue', 'green', 'black', 'yellow', 'cyan', 'brown', 'pink', 'purple', 'none'
    # 14.1.2 Main path
    mainpath = [
        'red']  # 'green', 'green', 'green', 'green', 'red', 'none', 'none', 'green', 'red', 'none', 'none', 'green'
